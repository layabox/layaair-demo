class RenderTextureDemo
{
	...
	onComplete() {
		...
		//加载场景
		let scene = Laya.stage.addChild(Laya.Loader.getRes("res/threeDimen/scene/CourtyardScene/Courtyard.ls"));
		//添加相机
		...
		//渲染到纹理的相机
		let renderTargetCamera = new Laya.Camera(0, 0.1, 1000);
		...
		//创建BlinnPhong材质
		let boxMaterial = new Laya.BlinnPhongMaterial();
		//创建纹理
		let boxTexture = new Laya.Texture2D(2048, 2048);
		//设置材质的纹理
		...
		Laya.loader.load(["res/threeDimen/ui/button.png"], Laya.Handler.create(this, function() {
			...
			changeActionButton.on(Laya.Event.CLICK, this, function() {
				...
				let boxMaterial = this.box.meshRenderer.material;
				//获取BlinnPhong材质的纹理
				let tex = boxMaterial.albedoTexture;
				//获取相机渲染目标的像素数据,默认renderTarget的颜色为RGBA
				let out = new Uint8Array(2048*2048*4); 
