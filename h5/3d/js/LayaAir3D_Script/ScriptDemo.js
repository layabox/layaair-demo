class ScriptDemo {
	constructor() {
		...
		//设置灯光漫反射颜色
		let lightColor = directionLight.color;
		...
		//添加自定义模型
		let box = scene.addChild(new Laya.MeshSprite3D(Laya.PrimitiveMesh.createBox(1, 1, 1), "MOs"));
		//设置模型的旋转
		box.transform.rotate(this.rotation2, false, false);
		//创建材质
		let material = new Laya.PBRSpecularMaterial();
		//加载模型的材质贴图
	...
	onLoop(box) {
		console.log("移除组件");
		// 获取到组件
		let boxContro = box.getComponent(BoxControlScript);
		// 移除组件
		boxContro.destroy();
...
class BoxControlScript extends Laya.Script3D {
    ...
    onStart() {
		//得到3D对象的材质
		let material = this._box.meshRenderer.material;
		//更改3D对象的材质反射率 （偏红）
