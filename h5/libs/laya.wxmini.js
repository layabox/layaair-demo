...
var MiniImage=(function(){
	...
	__proto._loadImage=function(url){
		...
		if (MiniAdpter.isZiYu){
			onCreateImage(url,thisLoader,true);
			return;
		};
		...
		if (!MiniFileMgr.getFileInfo(url)){
			if (url.indexOf('http://usr/')==-1&&(url.indexOf("http://")!=-1 || url.indexOf("https://")!=-1)){{
				...
				onCreateImage(url,thisLoader,!isTransformUrl);
			}
		}
		function onDownImgCallBack (sourceUrl,thisLoader,errorCode,tempFilePath){
		...
		function onCreateImage (sourceUrl,thisLoader,isLocal,tempFilePath){
